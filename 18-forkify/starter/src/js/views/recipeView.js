...
class RecipeView extends View {
  ...
  _formatQuantity(quantity) {
    ...
    const whole = Math.trunc(numerator / simplifiedDenominator);
    const remainder = numerator % simplifiedDenominator;

    if (!whole) return `${remainder}/${simplifiedDenominator}`;
    if (!remainder) return `${whole}`;
    return `${whole} ${remainder}/${simplifiedDenominator}`;
  }
